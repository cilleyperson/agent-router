...
@patch("httpx.AsyncClient.send")
def test_local_model_routing_redirection(mock_send):
    mock_resp = MagicMock()
    mock_resp.status_code = 200
    mock_resp.json.return_value = {
        "id": "chatcmpl-local123",
        "object": "chat.completion",
        "choices": [{"index": 0, "message": {"role": "assistant", "content": "This is a local reply from Ollama."}, "finish_reason": "stop"}],
        "usage": {"prompt_tokens": 10, "completion_tokens": 5}
    }
    ...
    with patch("main.load_config") as mock_load_config:
        ...
        with TestClient(app) as client:
            ...
            resp = client.post("/v1/chat/completions", json=payload)
            assert resp.status_code == 200
            assert resp.json()["choices"][0]["message"]["content"] == "This is a local reply from Ollama."
            
            assert mock_send.call_count == 1
            args, _ = mock_send.call_args
            request_obj = args[0]
            assert "http://localhost:11434/v1/chat/completions" in str(request_obj.url)
